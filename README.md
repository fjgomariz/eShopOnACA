...
The following Azure resources are provisioned by `azd up`:
- **Azure Container Apps Environment** — hosts the containerized web application
- **Azure Container App** — runs the eShopOnWeb web application as a container with external HTTP ingress
- **Azure Container Registry** — stores the Docker image built from the source code
- **Azure Key Vault** — securely stores application secrets (database connection strings, etc.)

**Notes:**
1. The application is deployed to an **Azure Container Apps Environment**. The Docker image is built and pushed to an **Azure Container Registry**, and then served via a Container App with an external ingress.
2. Considering security, we store its related data (id, password) in the **Azure Key Vault** when we create the database, and obtain it from the Key Vault when we use it. This is different from directly deploying applications locally.
3. The resource group name created in azure portal will be **rg-{env name}**.
